...
class WIC2Vec(utils.SaveLoad):
    ...
    def __init__(
            self, sentences=None, category_size = 10, word_size=20, alpha=0.025, window=3, min_count=0,
...
with open(input_file_name, 'rb') as f:
    pickle.load(teach_data, f)

articles = [(["a","b","c"], "i am man woman man get go"), (["b","c","d"], "get go sam meaning men")]
w = WIC2Vec()
w.build_vocab(articles)
for i in range(10):
    train_batch_sg(model = w, articles = articles, alpha = 0.0001)
